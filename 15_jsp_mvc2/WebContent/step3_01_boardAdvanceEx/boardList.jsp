...
<body>
	...
	<p align="right">
		<input type="button" value="테스트 데이터 생성" onclick="location.href='boardMakeDummyData.do'">
	</p>
			...
			<c:forEach var="boardAdvanceDto" items="${boardList }">
				<tr align="center">
					...
					<td align="left">
						...
						<a href="boardInfo.do?num=${boardAdvanceDto.num}">${boardAdvanceDto.subject}</a>
					</td>
			...
			<c:forEach var="i" begin="${startPage }" end="${endPage}">
				<c:if test="${currentPage <= allPageCount}">
					<c:if test="${i > 10 and i % 10 == 1}">
						<li><a href="boardList.do?currentPage=${startPage - 10 }&onePageViewCount=${onePageViewCount}&searchKeyword=${searchKeyword}&searchWord=${searchWord}">이전</a></li>
					</c:if>
					<li><a href="boardList.do?currentPage=${i }&onePageViewCount=${onePageViewCount}&searchKeyword=${searchKeyword}&searchWord=${searchWord}">${i }</a> &nbsp;</li>
					<c:if test="${i % 10 == 0 and i < allPageCount}">
						<li><a href="boardList.do?currentPage=${startPage + 10 }&onePageViewCount=${onePageViewCount}&searchKeyword=${searchKeyword}&searchWord=${searchWord}">다음</a></li>
					</c:if>
				</c:if>
			</c:forEach>
		</ul>
